...
public class S2Utils {
    ...
    public static <T extends Serializable> S2Response<S2> procesarPeticion(Supplier<Slice<T>> data, Function<T, S2> mapper, Integer rows) {
        return procesarPeticionConFiltro(data, mapper, rows, valor -> true);
    }
    
    public static <T extends Serializable> S2Response<S2> procesarPeticionConFiltro(Supplier<Slice<T>> data, Function<T, S2> mapper, Integer rows, Predicate<T> filtro) {//NOSONAR
